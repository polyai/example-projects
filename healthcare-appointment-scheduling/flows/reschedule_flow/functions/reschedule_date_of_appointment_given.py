...
@func_description(
    "Record the appointment date the user wants to reschedule as separate day, month, and year. Use N/A for missing parts when the year should be inferred from their loaded visits."
)
@func_parameter(
...
def reschedule_date_of_appointment_given(
    conv: Conversation,
    flow: Flow,
    ...
    log_prefix = (
        "[reschedule_date_of_appointment_given.reschedule_date_of_appointment_given]: "
    )
    raw = getattr(conv.state, "reschedule_upcoming_appointments", None) or []
    appointments = [Appointment.model_validate(x) for x in raw]
    plog.info(
        f"{log_prefix} reschedule_upcoming_appointments count={len(raw)} parsed_models={len(appointments)}"
    )
    ...
    if parse_err == "missing_day_or_month":
        ...
        return {"utterance": ("What date is the visit you're trying to reschedule?")}
    ...
    if len(matches) == 0:
        ...
        return {
            "utterance": (
                "I don't see an upcoming visit on that date. Which visit would you like to reschedule?"
            )
        }
    ...
    if len(matches) > 1:
        ...
        conv.write_metric("RESCHEDULE_FLOW_AMBIGUOUS_DATE", len(matches))
        ...
        if can_disambiguate_by_type:
            ...
            if can_disambiguate_by_time:
                ...
                return handoff(
                    conv,
                    reason="RESCHEDULE_AMBIGUOUS_DATE",
                    utterance="We found more than one visit on that day. I'll transfer you to a team member who can reschedule the right one.",
                )

        # Store matches for the disambiguation step
        conv.state.reschedule_ambiguous_matches = [
        ...
        return {
            "utterance": (
                ...
                f"Which one would you like to reschedule — {options}?"
            )
        }
    ...
    if not is_follow_up_appointment(chosen):
        conv.write_metric("RESCHEDULE_FLOW_NON_FOLLOW_UP", True)
        conv.write_metric("RESCHEDULE_OOS", True)
        return handoff(
            conv,
            reason="RESCHEDULE_NON_FOLLOW_UP",
            utterance=(
                "That type of visit can't be rescheduled on this line. "
    ...
    if chosen.is_rescheduled is True:
        ...
        conv.write_metric("RESCHEDULE_FLOW_ALREADY_RESCHEDULED", True)
    ...
    if not appt_id:
        ...
        conv.log.error("Reschedule flow: matched appointment has no id")
        conv.exit_flow()
        return {
            "utterance": "I'm sorry, but I wasn't able to match that appointment. Is there anything else I can help with today?",
        }
    ...
    plog.info(
        f"{log_prefix} set reschedule_target_appointment_id_last4="
        f"'{str(appt_id)[-4:] if len(str(appt_id)) >= 4 else '****'}'"
    )
    conv.log.info(
        "Reschedule flow: matched follow-up appointment",
