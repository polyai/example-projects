...
@func_description(
    "Find the caller's last (furthest out) upcoming eligible (follow-up) appointment and return its readable date so the agent can read it back and ask if that is the one they want to reschedule."
)
def suggest_last_reschedule_appointment(conv: Conversation, flow: Flow) -> dict:
    log_prefix = "[suggest_last_reschedule_appointment]: "
    ...
    if not eligible:
        ...
        conv.write_metric("RESCHEDULE_DATE_UNKNOWN_NO_ELIGIBLE_APPT", True)
        return handoff(
            conv,
    ...
    conv.write_metric("RESCHEDULE_DATE_UNKNOWN_SUGGESTED_LAST_APPT", True)
