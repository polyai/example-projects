...
@func_description(
    "Find the caller's next upcoming eligible (follow-up) appointment and return its readable date so the agent can read it back and ask if that is the one they want to cancel."
)
def suggest_next_cancel_appointment(conv: Conversation, flow: Flow) -> dict:
    log_prefix = "[suggest_next_cancel_appointment.suggest_next_cancel_appointment]: "

    raw = getattr(conv.state, "cancel_upcoming_appointments", None) or []
    appointments = [Appointment.model_validate(x) for x in raw]
    plog.info(f"{log_prefix} cancel_upcoming_appointments count={len(raw)}")
    ...
    type_label = appointment_type_label(next_appt.event_id)
    plog.info(
        f"{log_prefix} next_eligible_appointment_date='{str(next_appt.appointment_date)[:10]}'",
        is_pii=True,
    )
    conv.write_metric("CANCEL_DATE_UNKNOWN_SUGGESTED_NEXT_APPT", True)

    is_cm = getattr(conv.state, "caller_is_case_manager", False)
    if is_cm:
        return {
            "content": (
                f"The patient's next upcoming eligible visit is a {type_label} on {readable}. "
                f"Say something like: 'I can see the patient's next visit is a {type_label} on {readable} — is that "
    ...
    return {
        "content": (
            f"The patient's next upcoming eligible visit is a {type_label} on {readable}. "
            f"Say something like: 'I can see your next visit is a {type_label} on {readable} — is that "
