...
@func_description(
    "Find the caller's last (furthest out) upcoming eligible (follow-up) appointment and return its readable date so the agent can read it back and ask if that is the one they want to cancel."
)
def suggest_last_cancel_appointment(conv: Conversation, flow: Flow) -> dict:
    log_prefix = "[suggest_last_cancel_appointment]: "
