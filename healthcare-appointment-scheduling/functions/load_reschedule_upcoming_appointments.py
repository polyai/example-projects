...
class LoadRescheduleAppointmentsResult:
    """Result of loading appointments for reschedule flow."""

    ok: bool
    utterance: str


_LOG_PREFIX = "[load_reschedule_upcoming_appointments]: "


def load_reschedule_upcoming_appointments_for_state(
    conv: Conversation,
) -> LoadRescheduleAppointmentsResult:
    """
    Fetch upcoming visits for ``conv.state.identified_patient`` and store them on
    ``conv.state.reschedule_upcoming_appointments`` (JSON-serializable dicts).

    Call this **before** ``goto_flow("Reschedule Flow")`` so the flow can start at Resolve Appointment.
    ...
    if not patient or not pid:
        ...
        return LoadRescheduleAppointmentsResult(
            ok=False,
            utterance=(
                "We need to verify who you are before we can reschedule a visit. "
    ...
    conv.state.reschedule_upcoming_appointments = dumped
    conv.write_metric("RESCHEDULE_FLOW_APPOINTMENTS_LOADED", len(hydrated))
    conv.log.info(
        "Reschedule flow: loaded upcoming appointments (preload)",
        count=len(hydrated),
    ...
    plog.info(
        f"{_LOG_PREFIX} stored reschedule_upcoming_appointments count={len(dumped)}",
    ...
    if not hydrated:
        ...
        return LoadRescheduleAppointmentsResult(
            ...
            utterance=(
                ...
                "Would you like me to schedule a new appointment for you instead?"
            ),
        )

    plog.info(f"{_LOG_PREFIX} success hydrated_count={len(hydrated)}")
    return LoadRescheduleAppointmentsResult(ok=True, utterance="")


@func_description(
    "Preload upcoming appointments for reschedule flow (used from IDNV and start_rescheduling_flow)."
)
def load_reschedule_upcoming_appointments(conv: Conversation) -> None:
    """Platform entry point for this module (helpers are imported directly)."""
    log_prefix = "[load_reschedule_upcoming_appointments.load_reschedule_upcoming_appointments]: "
