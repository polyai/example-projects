"""Called when the caller identifies as a patient or case manager; sets state and continues to booking."""
...
@func_description(
    "Called when the caller answers whether they are a patient or a case manager calling on behalf of a patient. Sets caller type state and routes to new/existing patient check."
)
@func_parameter(
    "is_case_manager",
...
def caller_type_provided(conv: Conversation, flow: Flow, is_case_manager: bool):
    """Set caller type state variables and continue to Check New Patient."""
    ...
    flow.goto_step("Check New Patient")
    return {}
