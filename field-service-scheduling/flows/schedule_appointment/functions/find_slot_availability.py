...
@func_description(
    "check slot availability to schedule the user's appointment for, nb make sure that start_time and end_time are at least 2 hours apart"
)
@func_parameter(
    "date",
...
@func_parameter(
    "start_time",
    'Default to "NA". If the user wants to schedule for a specific time (or an arrival time no earlier than this), set to this time. Should be converted to HH:MM format (e.g. 13:10). If the user says "morning", set to "08:00" , if the user says "afternoon", set to "13:00". NOTE: If the user only says something like "early" or "earlier" without specific any time range then set this to "08:00"',
)
@func_parameter(
    "end_time",
...
@func_latency_control(
    ...
    delay_responses=[
        ("Just a couple of seconds please", 3),
        ("one more moment ... ", 3),
        ("okay, let's see...", 3),
    ],
)
def find_slot_availability(
    ...
    if getattr(conv.state, "USE_MOCK_API", False):
        ...
        return (
            ...
            f'Tell the user: "Looks like we can get someone out there on {slot_date}{timeframe}. '
            f'Would you like me to schedule your appointment for that time?"'
        )

    # Check if it's winter (December, January, or February) and if the requested time starts after 4pm or ends after 5pm
    is_winter = False
    is_too_late = False
    try:
        if date != "NA":
    ...
    if slot := get_potential_slot(
        ...
        duration="40" if conv.state.interior_needed == 2 else "20",
