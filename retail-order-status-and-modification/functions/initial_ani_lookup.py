...
def initial_ani_lookup(conv: Conversation):
    ...
    brand_name = conv.variant.get("brand_name") or conv.state.brand or "Poly Store"

    emergency_message = ""
    try:
        emergency_greeting = conv.real_time_config.get("emergency_greeting", {})
        if (
            isinstance(emergency_greeting, dict)
            and emergency_greeting.get("enabled")
            and emergency_greeting.get("message")
        ):
    ...
    return {
        "utterance": utterance(
            conv, "greeting", brand_name=brand_name, emergency_message=emergency_message
        )
    }
